...
namespace serializable
{
	...
	//std::vector<BloodCellType> predefinedCellModels {
	//	{
	...
	inline void to_json(json& j, const BloodCellType& definition)
	{
		j["name"] = definition.name;
		j["quantity"] = definition.quantity;
		for (auto& spring : definition.springs)
			j["springs"].push_back({ spring.first, spring.second });
	...
	inline void from_json(const json& j, BloodCellType& data)
	{
		data.name = j["name"];
